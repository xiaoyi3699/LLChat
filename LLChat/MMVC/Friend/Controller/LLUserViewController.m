...
- (void)loadUser {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        self.users = [[LLChatDBManager DBManager] users];
...
- (void)rightItemClick {
    NSString *uid = @"00002";
    LLChatUserModel *lastModel = self.users.lastObject;
    if (lastModel) {
    ...
    [self.tableView reloadData];
    //加入数据库
    [[LLChatDBManager DBManager] insertUserModel:model];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < self.users.count) {
        ...
        LLChatViewController *chatVC = [[LLChatViewController alloc] initWithUser:model];
