...
+ (instancetype)imageCache {
    static LLChatImageCache *imageCache;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        imageCache = [[LLChatImageCache alloc] init];
    });
    return imageCache;
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _cachePath = [LL_CHAT_APP_DocumentPath stringByAppendingPathComponent:@"LLChatImageCache"];
...
- (void)getImageWithUrl:(NSString *)url isUseCatch:(BOOL)isUseCatch completion:(void(^)(UIImage *image))completion{
    ...
    if (isUseCatch) {
        [self getImageFromCacheWithUrl:url completion:^(UIImage *image) {
            if (image) {
                completion(image);
            }
            else {
                [self getImageWithUrl:url completion:completion];
            }
        }];
    }
    else {
        [self getImageWithUrl:url completion:^(UIImage *image) {
            if (image) {
                completion(image);
            }
            else {
                [self getImageFromCacheWithUrl:url completion:completion];
            }
        }];
    }
}

- (void)getImageFromCacheWithUrl:(NSString *)url completion:(void(^)(UIImage *image))completion {
    //1、从内存获取
    NSString *urlKey = [url base64EncodedString];
    UIImage *image = [_memoryCache objectForKey:urlKey];
    if (image) {
